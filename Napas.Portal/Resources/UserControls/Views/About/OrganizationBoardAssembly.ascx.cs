...
{
    public partial class OrganizationBoardAssembly : UserControl
    {
        ...
        protected void Page_Load(object sender, EventArgs e)
        {
            if (!IsPostBack)
            {
                ...
                pageTitle += " - " + HttpContext.GetGlobalResourceObject("napas.resource", "GeneralAssembly").ToString();
                SetPageTitles(pageTitle);

                if (SPContext.Current.Web.Language == 1033)
                {
                    division = Constants.OrgNameEn.GeneralAssembly;
                    buttonName = "Detail";
                }
                else
                {
                    division = Constants.OrgNameVn.GeneralAssembly;
                    buttonName = "Chi tiết";
                }
                ...
                List<OrganizationBroadInfor> items = retOrg.getOrganizationBroad(web, division);
