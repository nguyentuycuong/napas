...
using NAPAS.Portal.Common.Framework.Core.Utils;

namespace NAPAS.Portal.Common.Framework.Core.WebParts
{
    [ToolboxItem(false)]
    public class ContainerWebPart : WebPart, IPostBackEventHandler, IValidator
    {
                    ...
                    if (!MustRenderFolderCreator())
                    {
                        ...
                        else
                        {
                            ...
                            catch (Exception ex)
                            {
                                ...
                                Controls.Add(new Literal { Text = string.Format("Container WebPart: {0}<br/>{1}", ex.Message, message) });
                            }
                        }
                    }
                    else
        ...
        private void SaveFolder()
        {
            ...
            if (string.IsNullOrEmpty(contentTypeId))
            {
                ...
                var folder = SPContext.Current.List.AddItem(rootFolder, SPFileSystemObjectType.Folder, folderName);
            ...
            {
                // Edit exist folder
                var folder = SPContext.Current.List.GetItemById(Convert.ToInt32(Page.Request.QueryString["ID"]));
                folder[SPBuiltInFieldId.Title] = folderName;
