...
namespace NAPAS.Portal.Common.Framework.Core.Helpers
{
    ...
    public abstract class BaseFieldCreator
    {
        ...
        {
            return XMLFieldFormat(guidLookupList, null);
        }

        public string XMLFieldFormat(string guidLookupList, string showField)
        {
            string xmlField = @"<Field " +
                                ...
                                (this.Type == SPFieldType.Lookup ? @"List='{" + guidLookupList + @"}' ShowField='" + showField + "' " : string.Empty) +
                                //(this.Type == SPFieldType.Lookup && showField != null ? @"ShowField='{" + guidLookupList + @"} '" : string.Empty) +
